...
const ModalUpdateTopping = ({ topping, onClose, onReload }: IData) => {
    ...
    const OnFinishForm = async () => {
        try {
            ...
            const thumbnailUri = thumbnail ? await UploadFile(thumbnail) : topping?.Image;
            const payload = {
                Id: topping?.Id,
                Name: formData?.Name,
            ...
            const res = await sendRequest<IResponse<ITopping>>({
                url: `${process.env.NEXT_PUBLIC_API_BACKEND}/odata/topping/id=${topping?.Id}`,
                method: "PUT",
                // headers: { Authorization: `Bearer ${accessToken}` },
                body: payload,
            })
        ...
        >
            <div className='my-4' >
                ...
                <Form.Item label="Giá (VNĐ)" name={"Price"} rules={[
                    ...
                    { type: 'number', min: 5000, max: 1000000, message: 'Giá phải từ 5,000 đến 1,000,000.' },
                ]} >
...
export default ModalUpdateTopping